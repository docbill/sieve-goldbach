...
\newcommand{\xPreMertens}{5416} % last data point with delta < 0 (from alpha=0.5 table)
\newcommand{\xMertens}{5557} % threshold for Lemma~\ref{lem:explicit-mertens} (from alpha=0.5 table)
\newcommand{\xDeltaMertens}{0.306962}  % gap at the explicit Mertens threshold \xMertens (from alpha=0.5 table)
\newcommand{\xNFivePercent}{41072} % n_5% threshold (from alpha=0.5 table)
\newcommand{\xNzeroStat}{71633} % lowest minimum past n_5% (OLD VALUE - table needs regeneration)
\newcommand{\xEtaStat}{0.2693} % delta at \xNzeroStat (OLD VALUE - table needs regeneration)
\newcommand{\xNasymStar}{7246}  % = \nasymstar (legacy value, may need update)
\newcommand{\xMinGlobal}{7219}       % argmin location of the empirical gap over the verified range (legacy value, may need update)
\newcommand{\xDeltaGlobal}{0.0526}   % min gap value at \xMinGlobal (legacy value, may need update)
...
\title{A Sieve-Theoretic Framework for Goldbach-Type Problems}
...
\begin{abstract}
A windowed sieve framework for the Goldbach problem is presented based on the quadratic form
\begin{equation}
Q(n,m)\;=\;(n-m)(n+m),
\end{equation}
...
Unconditionally, an explicit analytic baseline lower bound on the windowed Goldbach count \(\mathcal{G}(n;M)\) (with \(M=\lfloor n/2\rfloor\)) is proved via explicit Euler-Mertens products  with fully explicit constants, valid for all \(n\ge \xMertens\). A \emph{rescaling lemma} shows this bound holds uniformly for every smaller window \(M=\alpha n\) with \(0<\alpha\le \tfrac12\), and a monotonicity corollary extends it to larger windows by set inclusion.

Computationally, windowed Goldbach pairs are exhaustively enumerated for every even \(2n<23\#\) (where \(23\# = 223092870\)). 
Across seven decades, the normalized deviations from the parameter-free Hardy-Littlewood baseline (HL-A) decrease steadily. 
...
No unconditional resolution of Goldbach’s conjecture is claimed.
\end{abstract}
...
The Goldbach Conjecture~\cite{GoldbachSurvey,Ribenboim1991}, the Twin Prime Conjecture, and Polignac's Conjecture each address the distribution of prime pairs in different settings.  
A natural generalization emerges from considering these problems together: along suitable arithmetic or algebraic paths in the \( (n,m) \)-grid, prime pairs appear with a density consistent with the heuristic \( \#\mathcal{S}_N / \log^2 N \), leading to the following informal expectation.
...
Heuristically, beyond sufficiently large scale, one expects that for all large \( N \), every interval of length \( O(N) \) odd integers along that path contains at least \( O(N / \log^2 N) \) such prime pairs.

While this statement is unproven, it provides a coherent framework in which the problems above are special cases.  
...
\begin{enumerate}
  ...
  Normalized deviations between measured and predicted pair counts are defined as an evaluation across seven decades up to \(2n<23\#\). By the final decade the deviations fall below
  \( \xLambdaMinLimit \) (minimum), \( \xLambdaMaxLimit \) (maximum), and \( \xLambdaAvgLimit \) (average), with monotone decay across decades, providing the first large-data validation that windowed counts converge to the HL-A~\cite{HardyLittlewood1923} baseline, justifying extrapolation beyond the tested range.
  ...
  \cite{Mertens1874,RosserSchoenfeld1962, Dusart2010, HardyLittlewood1923, MontgomeryVaughan2007}
...
This work does not claim a resolution of Goldbach’s conjecture. Its purpose is to introduce a sieve-theoretic framework that isolates structural constraints shared by Goldbach-type problems and to demonstrate how existing bounds propagate within that framework.
...
Section~\ref{sec:stats} presents the computational study up to \(2n<23\#\), including primorial-interval convergence patterns.
...
A naive independence model would replace the factor
...
Figure~\ref{fig:CminmaxavgPrim} present the measured values compared to
the HL-A prediction lines for \( \alpha = 0.5, \).

\pgfplotstableread[col sep=comma, trim cells]{pairrangejoin-23PR.5-0.5-v0.1.6.csv}\pairdata
...
\end{axis}
\end{tikzpicture}
\caption{Scatter plots of \( \Cmeas_{\min} \), \( \Cmeas_{\max} \), and \( \Cmeas_{\tavg} \) versus \( n \) with HL-A prediction lines for \( \alpha = 0.5 \).   \emph{Maxima.} The prominent peaks occur at \(n\) whose odd part is a (multiple of a) primorial, in agreement with Proposition~\ref{prop:primorial-plateau}.}
...
Under HL-A, measured and predicted averages are heuristically expected to converge. For very large \( n \) this should approach \( 4 \). At the upper end of the tested range, asymmetries in the prime distribution above and below \( n \) still push the average slightly higher, but the correction factor \( \HLCorr \) accounts for this. Figure~\ref{fig:lambda-avg-prim} shows \( \Lambda_{\tavg} \) tending toward \( 0 \).

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdaavg-23PR.5-0.5-v0.1.6.csv}{n_geom}{Lambda_avg}
...
\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdamin-23PR.5-0.5-v0.1.6.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min} \rvert \)}{\( |\Lambda_{\min}| \) vs \( n_0 \) }
...
Predictions corrected by \( \HLCorr \) account for asymmetry in prime distribution. Figure~\ref{fig:lambda-maxprim} shows \( \Lambda_{\max} \to 0 \) with \( n \).
...
\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdamax-23PR.5-0.5-v0.1.6.csv}{n_1}{Lambda_max}{\( n_1 \)}{\( \lvert \Lambda_{\max} \rvert \)}{\( |\Lambda_{\max}| \) vs \( n_1 \) }
...
The sieve framework was tested against HL-A for all \( 2n < 23\# \) with \( \alpha = 0.5 \). The measured values \( \Cmeas_{\min},\Cmeas_{\max},\Cmeas_{\tavg} \) asymptotically approach predictions:
...
\pgfplotstableread[col sep=comma,trim cells]{gbpairsummary-23PR.5-empirical-cps-0.5-v0.1.6.csv}\cpsdata
...
The first recorded minimum above \(\CminusProduct\) after this crossing is taken as the conservative permanence threshold, \(\nprodstar = \xMertens\), with local margin
\(\zeta = \Cmeas_{\min}(\nprodstar) - \CminusProduct(\nprodstar) = \xDeltaMertens\).
Because only minima are recorded, the true last crossing may occur between these two recorded minima; consequently, \(\nprodstar\) represents a conservative (slightly late) bound on the permanence threshold.
...
(ii) The numerical value \(\eta=\xEtaStat\) represents the algebraically smallest deviation \(C(n)-\Cminus(n)\) observed after both \(n > n_{5\%}\) (statistical validity) and \(n > n_{\mathrm{preMertens}}\) (verified crossing below the Mertens bound); \(N_0=\xNzeroStat\) is the position where this minimum occurs.
...
\end{equation}
The last recorded minimum below \(\CminusProduct\) occurs at \(n=\xPreMertens\), and the first recorded minimum above \(\CminusProduct\) after that crossing occurs at \(\nprodstar=\xMertens\), where
\(\Delta(\nprodstar)=\xDeltaMertens>0\).
Since all observed values between \(\xPreMertens\) and \(\nprodstar\) also lie above \(\CminusProduct(n)\), this choice is conservative: \(\Delta(N_0)>0\) for all \(N_0\ge \nprodstar\), ensuring the analytic
...
Assuming \eqref{eq:SIBV}, there exists \(N_0\) such that every even \(N\ge N_0\) satisfies \(R_2(N)>0\).
In particular, any explicit bound \(N_0\) lying below the verified range \(2\nprodstar\) completes the reduction of Goldbach to \eqref{eq:SIBV}.
\end{corollary}
...
\begin{remark}[Checking Validity for Rescaling]
While Lemma~\ref{lem:alpha-rescale} allows rescaling from \(\alpha_0\) to any \(\alpha \le \alpha_0\), the Mertens certification threshold \(n_{\mathrm{Mertens}}\) depends on \(\alpha\). Table~\ref{tab:cps-alpha-summary} provides empirical Mertens positions and convergence statistics for 82 different \(\alpha\) values. Before applying the rescaling lemma, readers should consult this table to verify that the target \(\alpha\) has sufficient data support (i.e., \(n_{\mathrm{Mertens}}\) is well-separated from the dataset boundary) to justify the rescaling without requiring new certification computations. In practice, so long as \(n \ge n_{\mathrm{Mertens}}\) for the chosen \(\alpha\), the rescaling should yield a normalized lower bound of approximately \(\gtrsim 2\).
\end{remark}
...
\section{CPS Summary for Multiple Alpha Values}\label{app:cps-alpha}

Table~\ref{tab:cps-alpha-summary} presents the complete CPS (Certified Pair Summary) analysis for 82 different alpha values ranging from \( \alpha = 0.0009765625 \) to \( \alpha = 0.5 \), computed over the range \( 4 \le 2n < 23\# \). 

For each alpha value, the table shows:
\begin{itemize}
  \item \textbf{Alpha}: The window parameter \( \alpha \) where \( M = \alpha n \)
  \item \textbf{PreMertens, Mertens}: Positions where Mertens-type bounds are validated
  \item \textbf{DeltaMertens}: Normalized deviation at the Mertens position
  \item \textbf{n\_5percent}: The 5\% threshold position in the tested range
  \item \textbf{NzeroStat, EtaStat}: Statistical measures of minimum location and deviation
  \item \textbf{MertensAsymp, DeltaMertensAsymp}: Asymptotic analogs of the Mertens measures
  \item \textbf{NzeroStatAsymp, EtaStatAsymp}: Asymptotic statistical measures
\end{itemize}

\textbf{Certification Status:} Only \( \alpha = 0.5 \) has been fully certified in this work. The first five rows (\( \alpha < 0.0015 \)) exhibit edge effects where the minimum \( C(n) \) occurs close to the dataset boundary, providing insufficient data to reliably verify the Mertens condition. Values with \( \alpha \ge 0.0078125 \) may be suitable for certification but require individual inspection of convergence behavior. All data is provided for transparency and to enable independent verification by other researchers.

\bigskip
\bigskip

\begin{table}[H]
\caption{CPS summary statistics for multiple alpha values.}
\label{tab:cps-alpha-summary}
\end{table}
...
\pgfplotstabletypeset[
  ...
  columns={Alpha,PreMertens,Mertens,DeltaMertens,n_5precent,NzeroStat,EtaStat},
  columns/Alpha/.style={column name=\( \alpha \), precision=5, column type=l},
  columns/PreMertens/.style={column name=PreMertens, int detect, column type=r},
  columns/Mertens/.style={column name=Mertens, int detect, column type=r},
  columns/DeltaMertens/.style={column name=\( \Delta \)Mertens, precision=3, column type=r},
  columns/n_5precent/.style={column name=\( n_{5\%} \), int detect, column type=r},
  columns/NzeroStat/.style={column name=\( N_0 \)Stat, int detect, column type=r},
  columns/EtaStat/.style={column name=\( \eta \)Stat, precision=3, column type=r},
]{cpssummary-23PR.5-v0.1.6.csv}
\normalsize
...
\bibliography{goldbach-reformulation}
