...
Computationally, windowed Goldbach pairs are exhaustively enumerated for every even \(2n\le 2\cdot 10^8\). Normalized deviations between data and the parameter-free Hardy–Littlewood baseline (HL-A) \cite{HardyLittlewood1923} are \emph{reduced} across seven decades, with the final decade satisfying
\begin{equation}
|\Lambda_{\min}|\le \xLambdaMinLimit,\qquad
|\Lambda_{\max}|\le \xLambdaMaxLimit,\qquad
|\Lambda_{\tavg}|\le \xLambdaAvgLimit,
\end{equation}
providing large-scale validation of the HL–A model in this windowed setting. Observed blockwise maxima align with the singular-series structure: peaks occur when the odd part of \(n\) is a multiple of a primorial/2, yielding the expected primorial plateaus.
...
To begin, the certified bounds, the large–scale validation of the heuristic baseline, the window–scalability results, the structure of extrema, and the final conditional reduction to short–interval equidistribution, are summarized.
...
\begin{enumerate}\itemsep4pt
  ...
  \item \textbf{Calibrated Sieve–Heuristic and Per–Term Normalization.}
  The sieve–heuristic baseline on the structured family
  \(Q_m = n^2 - m^2\) is formalized, introducing a per–term normalization \(C_\star(n;I)\).
 ...
 Normalized deviations between measured and predicted pair counts are defined as an evaluation across seven decades up to \(2n=2\cdot 10^8\). 
...
\item \textbf{Certified Analytic (Shifted–Product) Lower Bound (Pairs).}
  ...
  \begin{equation}
    \mathcal{G}(n;M)\ \gtrsim\ \frac{K^2\,M}{\log{n}\,\log{\frac{3n}{2}}}.
  \end{equation}
  On the tested range this gives the concrete inequality
  \(\mathcal{G}(n;M)\ \ge \tfrac{2.1518\,M}{\log^2{n}}\).
  This certified bound is entirely independent of HL–A and does not rely 
  on \( \SGB \), or \( \betacal \).

  A sieve–theoretic lower bound for windowed Goldbach counts is certified. (Theorem~\ref{thm:main}). 
  ...
  The structure of extrema is analyzed via the singular series and identify record/local maximizers at odd primorials (Proposition~\ref{prop:primorial-plateau}). 
  Finally, under a short–interval equidistribution hypothesis strictly weaker than Hardy–Littlewood, pointwise positivity beyond an explicit cutoff (Corollary~\ref{cor:SI-BV}) is obtained, which, together with our computation, settles all cases.
...
This writing consists of multiple sections.  The Sieve-Heuristic Framework section: describes the Sieve Model, provides definitions, and presents heuristic relationships necessary to understand the framework used throughout this paper.   The Statistical--Analysis Section explores the model and compares it with data.  Minimal statistical assumptions are made to establish tight bounds on the constants measured, for use with practical analysis of Goldbach pairs.  Finally, the Sieve-Theoretic Golbach Conjecture derives a shifted-certified lower bound and a reduction to the short-interval Bombieri–Vinogradov hypothesis.
...
Let \(N_\ast\) denote the smallest \(N_0\) such that \(\Cmeas_{\min}(n) \ge \CminusProduct(n)\) for all subsequent \(n\) in the record.
...
The next recorded minimum is therefore taken as a conservative permanence threshold, \(N_\ast = \xMertens\), with local margin
...
\sum_{q\le x^{\theta}}
\ \max_{\substack{(a,q)=1}}\ \max_{x'\le x}\ \max_{H\ge x^{1/2+\varepsilon}}
...
Assuming \eqref{eq:SIBV}, there exists \(N_0\) such that every even \(N\ge N_0\) satisfies \(R_2(N)>0\). Together with our exhaustive verification up to \(2n_\ast\), this implies that every even integer \(>2\) is a Goldbach number.
\end{corollary}

\begin{proof}[Proof Outline With References]
Let \(N\) be large and even. Following the circle method (e.g.\ \cite[Ch.~13]{MontgomeryVaughan2007}, \cite[Chs.~17–18]{IwaniecKowalski2004}), introduce
\begin{equation}\label{eq:S-alpha}
S(\alpha)\ :=\ \sum_{n\le N}\Lambda(n)\,e(\alpha n),\qquad e(t):=e^{2\pi i t},
\end{equation}
so that
\begin{equation}\label{eq:R2-integral}
R_2(N)\ =\ \int_0^1 S(\alpha)^2\,e(-N\alpha)\,d\alpha.
\end{equation}
Decompose \([0,1]=\mathfrak M\cup\mathfrak m\) into \emph{major} and \emph{minor} arcs by rationals \(a/q\) with \(q\le Q\) (Farey dissection), where we fix \(Q=N^{1/2-\delta}\) for some small \(\delta>0\).

\medskip\noindent
\emph{Major arcs.} Standard evaluation of \(S(\alpha)\) on \(\mathfrak M\) gives
\begin{equation}\label{eq:major-arc}
\int_{\mathfrak M} S(\alpha)^2\,e(-N\alpha)\,d\alpha\ =\ \frac{\mathfrak S(N)\,N}{\log^2 N}\ +\ O\!\Bigl(\frac{N}{\log^3 N}\Bigr),
\end{equation}
with \(\mathfrak S(N)\) as in \eqref{eq:sing-series} (see \cite[Thm.~13.12]{MontgomeryVaughan2007}). Since \(\frac{p-1}{p-2}>1\) for every odd \(p\mid N\), we have
\begin{equation}\label{eq:S-lower}
\mathfrak S(N)\ \ge\ 2C_2.
\end{equation}

\medskip\noindent
\emph{Minor Arcs.} To bound \(\int_{\mathfrak m}S(\alpha)^2 e(-N\alpha)\,d\alpha\), one needs uniform cancellation in \(S(\alpha)\) when \(\alpha\) is well approximated by rationals \(a/q\) with \(q\le Q\). Expanding \(S(\alpha)\) by Vaughan’s identity and applying Cauchy–Schwarz together with the large sieve reduces the required bound to uniform estimates for primes in arithmetic progressions \emph{in short intervals}. The hypothesis \eqref{eq:SIBV} provides exactly that, with level of distribution \(\theta>1/2\) and interval length \(H\ge N^{1/2+\varepsilon}\). Consequently, for any fixed \(A>0\),
\begin{equation}\label{eq:minor-arc}
\int_{\mathfrak m} S(\alpha)^2\,e(-N\alpha)\,d\alpha\ \ll_{A,\varepsilon}\ \frac{N}{(\log N)^A}.
\end{equation}
(For this deduction see, the short–interval dispersion method in \cite[Chs.~17–18]{IwaniecKowalski2004} or \cite[Chs.~17,\,28]{Harman2007}.)

\medskip
Combining \eqref{eq:R2-integral}, \eqref{eq:major-arc}, and \eqref{eq:minor-arc} with \eqref{eq:S-lower} gives
\begin{equation}\label{eq:R2-lb-final}
R_2(N)\ \ge\ \frac{(2C_2)\,N}{\log^2 N}\ -\ K\,\frac{N}{(\log N)^A},
\end{equation}
for some \(K=K(A,\varepsilon)>0\). Choosing \(A\ge 3\) and \(N_0\) large so that
\begin{equation}\label{eq:N0-choice}
\frac{2C_2}{\log^2 N_0}\ >\ \frac{K}{(\log N_0)^A}
\end{equation}
forces \(R_2(N)>0\) for all even \(N\ge N_0\). The finite computation up to \(2n_\ast\) covers \(N< N_0\).
\end{proof}
...
For a given cardinality, the largest possible value of \(\prod f(p)\) (over sets of \emph{distinct} odd primes) is achieved by taking the \emph{initial segment} of odd primes \(\{3,5,\dots,p_y\}\); any attempt to replace one of these primes by a larger prime strictly decreases the product by \eqref{eq:f-monotone}. The smallest integer whose odd part contains exactly this set is the primorial \(P_y\), so among \(2n\le 2P_y\) the maximizer is \(n=P_y\), proving \eqref{eq:record-max}.
