...
In the seventh decade (\(10^7 \le n < 10^8\)) the \emph{raw decade maxima} already satisfy
\begin{equation}
...
Section~\ref{sec:sgb} contains the core sieve-theoretic results: the reduction lemma (Sec.~\ref{sec:reduction}), the certified lower bound (Thm.~\ref{thm:main}), its conditional corollary under short-interval equidistribution (Sec.~\ref{sec:SI-BV}), and the primorial maxima proposition (Sec.~\ref{sec:primorial-prop}).
...
\subsection{Primorial plateaus under the HL–A baseline}\label{sec:primorial-prop}

% Half-primorial notation (optional, if not already defined)
...
Let \(P_y=\prod_{3\le p\le p_y}p\) be the half-primorial up to the odd prime \(p_y\), and let \(p_{y+1}\) be the next odd prime. Then:
...
P_y\ \le\ n\ <\ p_{y+1}\,P_y,
\end{equation}
the maximum of \(\mathfrak S(2n)\) is attained precisely at those \(n\) for which all odd primes \(\{3,5,\dots,p_y\}\) divide \(n\); equivalently, \(P_y\mid n\) (up to a power of \(2\)). All such \(n\) have the same value of \(\mathfrak S(2n)\).
\end{proposition}
...
f(q)\ <\ f(p),
\end{equation}
i.e. the multiplicative gain from including a prime strictly decreases with the prime’s size.

For a given size budget, the largest possible value of \(\prod f(p)\) (over sets of \emph{distinct} odd primes) is achieved by taking the \emph{initial segment} \(\{3,5,\dots,p_y\}\); replacing any of these by a larger prime strictly decreases the product by \eqref{eq:f-monotone}. The smallest integer carrying exactly this set is \(P_y\), so among \(2n\le 2P_y\) the maximizer is \(n=P_y\), proving \eqref{eq:record-max}.

On the interval \eqref{eq:plateau-interval}, no \(n\) can introduce a new \emph{distinct} odd prime beyond \(p_y\) (that would force \(n\ge p_{y+1}P_y\)). Thus every \(n\) in the interval has the same \emph{set} of odd prime divisors as some divisor of \(P_y\); \(\mathfrak S\) is maximized precisely when that set equals \(\{3,5,\dots,p_y\}\), i.e. when \(P_y\mid n\) up to a power of \(2\). Exponents do not affect \(\mathfrak S\), so any multiple \(k\cdot P_y\) with \(k\) composed only of primes from \(\{3,5,\dots,p_y\}\) has the same \(\mathfrak S\) as \(P_y\). This gives the plateau and completes the proof.
\end{proof}
...
\begin{enumerate}
   ...
   corresponding verification checksums are included with this submission.\footnote{This explicit verification up to \( \nprodstar \) is complementary to large-scale computational results such as Oliveira e Silva, Herzog, and Pardi [OeSHP2014], who verified Goldbach’s conjecture for all even integers up to \( 4\cdot 10^{18} \). Our approach is distinct in that it provides a certified sieve-theoretic lower bound valid for all \( n \ge \nprodstar \), thereby bridging analytic proof and computational verification.}
...
We therefore take the next recorded minimum as a conservative permanence threshold, \(\nprodstar = \xMertens\), with local margin
...
\[
\nasymstar \ :=\ \min\Big\{n\ge \nprodstar:\ \CminusAsymp(n)\le \CminusProduct(n)\ \text{and}\ 
\Cmeas_{\min}(n')\ \ge\ \CminusAsymp(n')\ \ \forall\, n'\ge n\Big\}.
\]
In our dataset, \(\nasymstar=\xNasymStar\).
\end{remark}
