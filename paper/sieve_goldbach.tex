...
\newcommand{\xSecondLambdaMaxLimit}{1.2\cdot 10^{-3}} % Decade 8, LambdaAvg2ndAbsMax value
...
\newcommand{\xLambdaMaxLimit}{1.3\cdot 10^{-3}} % Decade 8, LambdaAvg2ndAbsMax value
...
% --- Bin markers (centralized so you can swap styles later) ---
\newcommand{\PrimTag}{\scriptscriptstyle{\#}}     % primorial marker
\newcommand{\DecTag}{\scriptscriptstyle{D}}       % decimal marker
...
% ---------- Measured (subscript first, then superscript with marker) ----------
\NewDocumentCommand{\NmeasDec}{e{^_}}{%
  \Nmeas%
  \IfValueT{#2}{_{#2}}%
  ^{\DecTag\IfValueT{#1}{\,#1}}%
}
\NewDocumentCommand{\NmeasPrim}{e{^_}}{%
  \Nmeas%
  \IfValueT{#2}{_{#2}}%
  ^{\PrimTag\IfValueT{#1}{\,#1}}%
}
\NewDocumentCommand{\CmeasDec}{e{^_}}{%
  \Cmeas%
  \IfValueT{#2}{_{#2}}%
  ^{\DecTag\IfValueT{#1}{\,#1}}%
}
\NewDocumentCommand{\CmeasPrim}{e{^_}}{%
  \Cmeas%
  \IfValueT{#2}{_{#2}}%
  ^{\PrimTag\IfValueT{#1}{\,#1}}%
}

% ---------- Predicted (same pattern) ----------
\NewDocumentCommand{\NpredDec}{e{^_}}{%
  \mathring{n}%
  \IfValueT{#2}{_{#2}}%
  ^{\DecTag\IfValueT{#1}{\,#1}}%
}
\NewDocumentCommand{\NpredPrim}{e{^_}}{%
  \mathring{n}%
  \IfValueT{#2}{_{#2}}%
  ^{\PrimTag\IfValueT{#1}{\,#1}}%
}
\NewDocumentCommand{\CpredDec}{e{^_}}{%
  \mathring{C}%
  \IfValueT{#2}{_{#2}}%
  ^{\DecTag\IfValueT{#1}{\,#1}}%
}
\NewDocumentCommand{\CpredPrim}{e{^_}}{%
  \mathring{C}%
  \IfValueT{#2}{_{#2}}%
  ^{\PrimTag\IfValueT{#1}{\,#1}}%
}
...
% Generic block machinery
\newcommand{\Bgeom}[2]{\left\lfloor \sqrt{#1\,(#2-1)}\right\rceil} % nearest integer to sqrt(ab-a)
% Euler-cap macro (roman font, works in text and math)
...
\newcommand{\FigLambdaAbsSignSplit}[6]{%
  ...
  \pgfplotstableread[col sep=comma,trim cells]{\CSVPATH}\datatable
  \begin{tikzpicture}
  \begin{axis}[
    xmin=10,
   ...
   \foreach \x in {15,105,1155,15015,255255,4849845,111546435}{
       \addplot[primorial, forget plot] coordinates {(\x,1) (\x,1e-4)};
   }
...
\begin{definition}[Euler-cap limit and admissible window parameter]
For a given central value \(n\), define the \emph{Euler cap} by
\begin{equation}\label{eq:euler_cap}
\Ecap(n)\ :=\ \frac{(2n+1) - \sqrt{8n+1}}{2n}.
\end{equation}
The Euler cap quantifies the fractional portion of a symmetric window
\([n-M,n+M]\) for which both factors \((n-m)\) and \((n+m)\) remain positive
and distinct.  Beyond this limit, the correspondence between prime pairs
\((n-m),(n+m)\) and their semiprime product \((n-m)(n+m)\) no longer holds.

For any window parameter \(\alpha\in(0,1]\), we therefore define
\begin{equation}
M\ :=\ \bigl\lfloor\,\min\bigl(\alpha,\Ecap(n)\bigr)\,n\,\bigr\rfloor.
\end{equation}
The choice \(\alpha=\Ecap(n)\) gives the maximal window allowed by the Euler-cap constraint,
while for practical computations we fix \(\alpha\le1\).
Since \(\Ecap(n)\to1\) as \(n\to\infty\),
the case \(\alpha=1\) is fully admissible asymptotically
and can be used safely in tabulated results.
\end{definition}
...
\smallskip
\noindent
For \textbf{decimal-based binning}, each block is defined as
\begin{equation}
B_{d,k}=[d\cdot10^k,(d+1)\cdot10^k),
\qquad 1\le d\le 9,\ k\in\mathbb{Z}.
\end{equation}
Within each block,
\begin{equation}
\NmeasDec_0:=\arg\min_{n\in B_{d,k}}\Cmeas(n;I),\qquad
\NmeasDec_1:=\arg\max_{n\in B_{d,k}}\Cmeas(n;I),
\end{equation}
\begin{equation}
\CmeasDec_{\min}(d,k):=\Cmeas(\NmeasDec_0;I),\quad
\CmeasDec_{\max}(d,k):=\Cmeas(\NmeasDec_1;I),\quad
\CmeasDec_{\tavg}(d,k):=\frac{1}{\card{B_{d,k}}}\sum_{n\in B_{d,k}}\Cmeas(n;I).
\end{equation}

\smallskip
\noindent
For \textbf{primorial-based binning}, we treat small values separately:
for \(n<15\), bins are singletons \([k,k+1)\), \(k=1,\dots,14\); 
for \(15\le n<30\), a single block \([15,30)\) is used.

For \(n\ge30\), let \(p<q\) be consecutive primes with corresponding primorials \(p\#\) and \(q\#\).
...
\end{equation}
forming consecutive intervals of width \(w_p\), with the final bin truncated so that its right endpoint is exactly \(q\#\).

Within each primorial bin \(B_{j,p\#}\),
\begin{equation}
\NmeasPrim_0:=\arg\min_{n\in B_{j,p\#}}\Cmeas(n;I),\qquad
\NmeasPrim_1:=\arg\max_{n\in B_{j,p\#}}\Cmeas(n;I),
\end{equation}
\begin{equation}
\CmeasPrim_{\min}(j,p\#):=\Cmeas(\NmeasPrim_0;I),\quad
\CmeasPrim_{\max}(j,p\#):=\Cmeas(\NmeasPrim_1;I),\quad
\CmeasPrim_{\tavg}(j,p\#):=\frac{1}{\card{B_{j,p\#}}}\sum_{n\in B_{j,p\#}}\Cmeas(n;I).
\end{equation}
\end{definition}

\begin{remark}
Both binning schemes provide a consistent means of defining long-interval aggregates,
which are the only scales on which \(\Cmeas\) is statistically meaningful.
Primorial-based binning produces smoother results and fewer boundary artifacts,
and is therefore preferred for analytical plots.
Decimal-based binning, using one significant digit per block,
is better suited for comprehensive tabulation owing to its smaller and more uniform number of rows.
\end{remark}
...
The windowed logarithmic effect is absorbed into the prediction via a Hardy--Littlewood circle correction factor:
\begin{equation}
\HLCorr(n;I)
\;:=\;
\frac{\log^2 n}{\card{\Ipar}}\,
\sum_{m\in \Ipar}\frac{1}{\log(n-m)\,\log(n+m)}\,,
\end{equation}
for all \(n\) and \(I\) such that \(n\pm m\ge3\) for all \(m\in\Ipar\).

Fix a window \(I\subset\mathbb{Z}\setminus\{0\}\) with \(M:=\card{\Ipar}\).
Let \(C_\star(n;I)\) denote the per-term HL-normalized constant (unordered scale),
and define the \emph{predicted (pairs-scale) constant}
\begin{equation}
...
\medskip
\noindent
\textbf{Block formulation.}
For any finite block \(B=[a,b)\subset\mathbb{N}\) with \(\card{B}\ge1\), define extremizers
(by \(C_\star\), equivalently by \(\Cpred/\HLCorr\)):
\begin{equation}
\Npred_0(B) := \arg\min_{n\in B} \frac{\Cpred(n;I)}{\HLCorr(n;I)},\qquad
\Npred_1(B) := \arg\max_{n\in B} \frac{\Cpred(n;I)}{\HLCorr(n;I)}.
\end{equation}
Then set
\begin{equation}
\Cpred_{\min}(B):=\Cpred\bigl(\Npred_0(B);I\bigr),\qquad
\Cpred_{\max}(B):=\Cpred\bigl(\Npred_1(B);I\bigr),
\end{equation}
and approximate the block average by a two-point proxy of the slowly varying \(\HLCorr\):
\begin{equation}
\Cpred_{\tavg}(B)
\;:=\;
\frac{\HLCorr(\Ngeom;I)+\HLCorr(\Ngeom+1;I)}{2\,\card{B}}
\sum_{n\in B}\frac{\Cpred(n;I)}{\HLCorr(n;I)},
\qquad
\Ngeom:=\Bgeom{a}{b}.
\end{equation}
(Optionally, \(\Ngeom\) may be restricted to the nearest \emph{odd} integer.)

\medskip
\noindent
\textbf{Specialization to primorial blocks.}
Without loss of generality, we take blocks aligned to consecutive primorial plateaus.
For \(p<q\) consecutive primes, let \(p\#,q\#\) be the corresponding primorials.
Define the bin width and count
\[
w_p := \max\!\Bigl(15,\frac{p\#}{2p}\Bigr), \qquad
L_p := q\# - p\#, \qquad
K_p := \Bigl\lceil \frac{L_p}{w_p} \Bigr\rceil,
\]
and nodes \(a_j := p\# + \min(j\,w_p,L_p)\), \(j=0,1,\dots,K_p\),
so that \(a_0=p\#\) and \(a_{K_p}=q\#\).
The indexed primorial blocks are
\begin{equation}
B_{j,p\#}:=[a_j,a_{j+1}), \qquad j=0,1,\dots,K_p-1.
\end{equation}
For each block \(B_{j,p\#}\),
\begin{equation}
\NpredPrim_0 := \Npred_0(B_{j,p\#}),\qquad
\NpredPrim_1 := \Npred_1(B_{j,p\#}),
\end{equation}
\begin{equation}
\CpredPrim_{\min}(j,p\#):=\Cpred_{\min}(B_{j,p\#}),\quad
\CpredPrim_{\max}(j,p\#):=\Cpred_{\max}(B_{j,p\#}),\quad
\CpredPrim_{\tavg}(j,p\#):=\Cpred_{\tavg}(B_{j,p\#}).
\end{equation}

\begin{remark}
This structure generalizes naturally to any block family.
Primorial-based blocks are preferred, as they align with multiplicative symmetries
and minimize boundary artifacts.
Decimal blocks \(B_{d,k}=[d\!\cdot\!10^k,(d+1)\!\cdot\!10^k)\)
may still be used for tabular summaries where fewer rows are desirable.
\end{remark}
\end{definition}
...
\subsection{Alignment Correction (with partial exposure of the next prime)}

The Hardy–Littlewood Conjecture~A is a model designed to capture
point-wise averages.  When applied to predict minima, the values are
typically overestimated, and the predicted convergence to the mean is
too rapid.  The Chinese Remainder Theorem (CRT) offers a means to
estimate how large a deviation from the Hardy–Littlewood minimum might
be possible through phase alignment of the small primes.

The difficulty is that the CRT applies to \emph{prefiltered}
candidates—integers already reduced modulo the small primes.  Over a
full interval one must reintroduce the averaging over those filters,
and in doing so loses knowledge of the maximal deviation produced by
alignment.  To avoid this dilemma, we apply the CRT on a \emph{short}
interval of length \( \sqrt{n} \).  This length is large enough to
include a full period of the small primes, yet short enough that it is
plausible for the entire set of surviving candidates to be eliminated
through a coincidental alignment of residues.

Let \(Q(n)\) denote the largest odd primorial (product of odd primes)
whose period fits into twice the window, and let \(p_{\text{next}}(n)\) be
the next odd prime:
\begin{equation}
Q(n) =  \prod_{3\le p<y(n)} p,
\qquad
Q(n)\le \sqrt{2n} < Q(n)\,p_{\text{next}}(n).
\end{equation}
The “base” small-prime alignment budget cancels
\begin{equation}
R_{0}(n) = \prod_{3\le p<y(n)}\!(p-2).
\end{equation}
For heuristic convenience one may use the smooth proxy
\begin{equation}
R_{0}(n) \approx \frac{Q(n)}{\log y(n)}
 \asymp  \frac{\sqrt{n}}{\bigl(\log!\log n\bigr)^{2}},
\end{equation}
valid at the level of orders of magnitude for \(Q(n)\approx\sqrt{2n}\).

Although \(Q(n)\) is the last \emph{full} period inside the window, the
window still covers a fraction of the next period of length
\(Q(n),p_{\text{next}}(n)\).  Writing
\begin{equation}
s(n) = \frac{\sqrt{2n}}{Q(n)\,p_{\text{next}}(n)} \in (0,1),
\end{equation}
the alignment leverage contributed by the next prime is well modeled by
a fractional factor
\begin{equation}
\bigl(p_{\text{next}}(n)-2\bigr)^{\,s(n)} = \exp\Bigl(s(n)\,\log\!\bigl(p_{\text{next}}(n)-2\bigr)\Bigr).
\end{equation}
There is \emph{no} extra leverage from this next prime when it divides
the center \(n\), so we set the factor to \(1\) in that case.  Thus the
effective alignment budget becomes

\begin{equation}
R_{\text{eff}}(n) = R_{0}(n) \times
\begin{cases}
\bigl(p_{\text{next}}(n)-2\bigr)^{s(n)}, & \text{if } p_{\text{next}}(n)\nmid n,\\[4pt]
1, & \text{if } p_{\text{next}}(n)\mid n.
\end{cases}
\end{equation}

Empirically this “partial credit” improves agreement by roughly an order
of magnitude compared to using \(R_{0}(n)\) alone, while remaining
consistent with the discrete tenting picture (the factor interpolates
between adjacent certainty steps).

The fundamental assumption is that a cancelled subinterval can, on occasion,
lie entirely within the observation window, producing a potential—but not
necessarily realized—minimum.  A factor of two is included to account for
ordered pairs.  To prevent the appearance of negative values, the alignment
correction is applied as a lower envelope bounded below by zero:
\begin{equation}
\Cpred_{\min}^{\talign}(I) = \min_{n\in I} \max\!\Biggl( 0,\Cpred(n) - \frac{2\,\log^{2}\! n} {\alpha\,\sqrt{n}\bigl(\log\!\log n\bigr)^{2}} \times \Psi(n) \Biggr),
\label{eq:Cpred_align}
\end{equation}
where the partial-exposure multiplier \(\Psi(n)\) encodes the next-prime logic:
\begin{equation}
\Psi(n) = 
\begin{cases}
\exp\!\Bigl(s(n)\,\log\!\bigl(p_{\text{next}}(n)-2\bigr)\Bigr), 
& \text{if } p_{\text{next}}(n)\nmid n,\\[4pt]
1, 
& \text{if } p_{\text{next}}(n)\mid n.
\end{cases}
\end{equation}
(Equivalently, \(\Psi(n)\in[1,p_{\text{next}}(n)-2)\) with the upper
bound unattained because \(s(n)!<!1).\)

For heuristic analysis the smooth proxy \(R_{0}(n)\) multiplied by
\(\Psi(n)\) is more useful than the exact discrete product: the discrete
step function captures only cases where a full next-period remainder is
known to exist, whereas the proxy describes the regime in which such an
alignment may or may not occur—and does so in a way that closely tracks
empirical minima without overstating the effect.

Because the location of a potential cancellation within the observation window is unknown, the Hardy–Littlewood circle correction maybe applied in several ways to normalize the alignment term.  One may take the maximum circle correction that occurs anywhere within the window, the minimum correction, or simply reuse the same correction already applied to the window itself:
\begin{equation}
\Phi_{\max}(I)=\max_{n\in I}\HLCorr(n,\delta(n)),\qquad
\Phi_{\min}(I)=\min_{n\in I}\HLCorr(n,\delta(n)),\qquad
\Phi_{\twin}(n)=\HLCorr(n,\delta(n)).
\end{equation}
Since \( \HLCorr(\cdot,\cdot)\ge1 \) and varies slowly with \( n \),
these choices satisfy
\( \Phi_{\min}(I)\le\Phi_{\twin}(n)\le\Phi_{\max}(I) \).
Multiplying the alignment term by \( \Phi \) gives a normalized
subtraction
\begin{equation}
A_{\Phi}(n) = \frac{2\,\log^{2}! n} {\alpha\,\sqrt{n}\,\bigl(\log\!\log n\bigr)^{2}}\,\Psi(n)\,\Phi,
\end{equation}
where \( \Psi(n) \) encodes the partial exposure of the next prime
as described above.
For rigorous lower–bound certification it is most conservative to use
the maximal correction \( \Phi_{\max}(I) \), which ensures that no
underestimation of the true minimum can occur.
For empirical comparisons, however, the window correction
\( \Phi_{\twin}(n) \) provides the best agreement with the measured data,
at the cost of occasionally overestimating the minimum.
Finally, the minimal correction \( \Phi_{\min}(I) \) serves as a useful
reference for testing sensitivity to the choice of normalization.

With these choices, the lower-envelope formula
\eqref{eq:Cpred_align} becomes, for a selected \(\Phi\),
\begin{equation}
  \Cpred_{\min}^{\talign,\Phi}(I) = \min_{n\in I} \max\!\Bigl(0,\;\Cpred(n)\;-\;A_{\Phi}(n)\Bigr),
\end{equation}
making explicit the trade–off between conservative certification
\(\Phi_{\max}\) and empirical fidelity \(\Phi_{\twin}\).

\subsection{Working Hypotheses}

The alignment framework introduced above relies on several hypotheses
that are consistent with both the analytic reasoning and the observed
data.  These are presented here explicitly to distinguish what is
\emph{assumed} from what is \emph{derived}.

\begin{hypothesis}[Short-Interval Alignment Hypothesis]
Within a short interval of length~\( \sqrt{n} \), the residues of small
primes can occasionally align in such a way that all surviving candidates
of the Hardy--Littlewood sieve are cancelled.  This phenomenon defines
a possible, though not guaranteed, local minimum of
\( \Cmeas(n) \).
\end{hypothesis}

\begin{remark}[Short-Interval Hypothesis Conflict]
At first glance the alignment correction appears to conflict with the 
short-interval Bombieri-Vinogradov equidistribution hypothesis 
(Sec.~\ref{sec:SI-BV}.  The alignment term grows faster than
the expected number of candidate pairs, providing a mechanism that
could in principle allow infinitely many short intervals devoid of
Goldbach pairs.  This is the same paradox that arises if the complete
CRT is applied to the full interval.  However, nothing in this argument
precludes an additional damping mechanism that limits such effects at
large~\(n\).  Simple statistical reasoning may show that the required
alignments become exponentially rare, rendering their contribution
negligible.  Thus, while the success of the alignment correction makes
the short-interval hypotheses appear less secure, any empirical
agreement should be interpreted as heuristic rather than conclusive.
\end{remark}

\begin{hypothesis}[Partial-Exposure Hypothesis]
The leverage of the next prime beyond the last full odd primorial
contributes fractionally according to the portion of its period
contained in the window:
\begin{equation}
  \Psi(n) = \exp\!\Bigl(s(n)\,\log\!\bigl(p_{\text{next}}(n)-2\bigr)\Bigr),
  \qquad
  s(n) = \frac{\sqrt{2n}}{Q(n)\,p_{\text{next}}(n)}.
\end{equation}
If \(p_{\text{next}}\mid n\), this leverage is null, \(\Psi(n)=1\).
\end{hypothesis}

\begin{hypothesis}[Lower-Envelope Hypothesis]
The minimal expected number of Goldbach-compatible pairs in any finite
window is given by a lower envelope
\begin{equation}
  \Cpred_{\min}^{\talign}(I)
  = \min_{n\in I}
    \max\!\Bigl(
      ...
      \Cpred(n)
      - \frac{2\,\log^{2}\!n}
             {\alpha\,\sqrt{n}\,(\log\!\log n)^{2}}
      \Psi(n)
    \Bigr),
\end{equation}
where \( \alpha \) controls the observation window.
\end{hypothesis}

\begin{remark}
The empirical evidence presented in Section~\ref{sec:results}
supports these hypotheses within one order of magnitude across the
verified range.  The alignment hypothesis explains the observed
over-prediction of Hardy--Littlewood minima and the apparent tenting
structure near primorial boundaries, while the partial-exposure
hypothesis reduces residuals by approximately an additional order of
magnitude.  Each hypothesis remains falsifiable by direct computation
at larger~\( n \).
\end{remark}
...
For numerical stability, Goldbach pairs \( (n-m,\,n+m) \) are evaluated over
a symmetric range \( m \in [-M,\,M]\setminus\{0\} \), where
\begin{equation}
  M = \bigl\lfloor \min\!\bigl(\alpha,\Ecap(n)\bigr)\,n \bigr\rfloor,
\end{equation}
and the scaling parameter \( \alpha \) takes values
\( 2^{k/8} \) for \( k \in [0,80] \).
The Euler cap \( \Ecap(n) \), defined in~\eqref{eq:euler_cap},
represents the upper bound on \( m/n \) for which both the
Hardy–Littlewood circle correction and the semiprime–to–prime pair
equivalence remain valid.
The symmetric formulation improves numerical conditioning and ensures
predictable convergence, although the same framework is equally
applicable to asymmetric or alternative interval choices.

The programs used to generate primes and sieve the data were written in
C, C++, and AWK, and executed on an Intel i5 processor in a 2015 laptop
serving as a home server.  The full computation of Goldbach pairs up to
\( n = 23\# / 2 \) requires approximately one week using multiple
parallel processes, while partial results and predicted values can be
produced within minutes.  All \( \alpha \)-values are computed within a
single process.  The source code is released under GPL-3.0-or-later, and
the manuscript under CC-BY-4.0.  All source code and certified datasets
are permanently archived on Zenodo~\cite{Riemers2025SieveGoldbach}.

The measured variables
\( \Nmeas_0, \Nmeas_1, \Cmeas_{\min}, \Cmeas_{\max}, \Cmeas_{\tavg} \)
are defined in Definition~\ref{semiprime-survivors}.
Appendix Table~\ref{tab:raw-stats} provides representative
unnormalized decade data for \( \alpha = 0.5 \), sufficient for
independent verification of the results.
The reported minima of zero pairs for \( n = 7, 11, \) and \( 43 \)
arise from the exclusion of specific pairs such as \( (7,7) \),
\( (3,19) \), and \( (7,79) \) by the chosen window and therefore do not
contradict Goldbach's Conjecture.

Figures~\ref{fig:CminmaxavgPrim1},~\ref{fig:CminmaxavgPrim}, and
\ref{fig:CminmaxavgPrim015625} present the measured values compared to
the HL-A prediction lines for \( \alpha = 1, 0.5, \) and \( 0.015625 \),
respectively.
...
\addplot[mypred, thick, purple!70!black, dotted, sharp plot]
  table[x={n_a}, y={Cbound}] {\pairdata};
\addlegendentry{\( \Cpred_{\min}^{\tbound} \)}

\addplot[mypred, thick, brown!70!black, dashed, sharp plot]
  table[x={n_v}, y={Calign_min}] {\pairdata};
\addlegendentry{\( \Cpred_{\min}^{\talign} \)}

\addplot[mypred, thick, orange!70!black, dashed, sharp plot]
  table[x={n_u}, y={Cpred_max}] {\pairdata};
\addlegendentry{\( \Cpred_{\max}^{\talign} \)}
...
\addplot[mypred, red!70!black, sharp plot]
...
\end{axis}
\end{tikzpicture}
\caption{Scatter plots of \( \Cmeas_{\min} \), \( \Cmeas_{\max} \), and \( \Cmeas_{\tavg} \) versus \( n \) with HL-A prediction lines for \( \alpha = 1 \).   \emph{Maxima.} The prominent peaks occur at \(n\) whose odd part is a (multiple of a) primorial, in agreement with Proposition~\ref{prop:primorial-plateau}.    The \( \Cpred_{\min}^{\talign} \) curve represents the maximum amount of cancellation that can result from small primes from CRT.}
...
\end{axis}
\end{tikzpicture}
\caption{Scatter plots of \( \Cmeas_{\min} \), \( \Cmeas_{\max} \), and \( \Cmeas_{\tavg} \) versus \( n \) with HL-A prediction lines for \( \alpha = 0.5 \).   \emph{Maxima.} The prominent peaks occur at \(n\) whose odd part is a (multiple of a) primorial, in agreement with Proposition~\ref{prop:primorial-plateau}.    The \( \Cpred_{\min}^{\talign} \) curve represents the maximum amount of cancellation that can result from small primes from CRT.}
...
\end{axis}
\end{tikzpicture}
\caption{Scatter plots of \( \Cmeas_{\min} \), \( \Cmeas_{\max} \), and \( \Cmeas_{\tavg} \) versus \( n \) with HL-A prediction lines for \( \alpha = 0.015625 \).   \emph{Maxima.} The prominent peaks occur at \(n\) whose odd part is a (multiple of a) primorial, in agreement with Proposition~\ref{prop:primorial-plateau}.    The \( \Cpred_{\min}^{\talign} \) curve represents the maximum amount of cancellation that can result from small primes from CRT.}
...
\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdaavg-23PR.5-1-v0.2.0.csv}{n_geom}{Lambda_avg}
    {\( \Ngeom \)}{\( \lvert \Lambda_{\tavg} \rvert \)}
    {\( \vert \Lambda_{\tavg} \rvert \) vs \( \Ngeom \)}
  \caption{Scatter plot of \( \lvert \Lambda_{\tavg} \rvert \) versus \( \Ngeom \) for \( \alpha=1 \) on a log-log scale.}
  \label{fig:lambda-avg-prim1}
\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdaavg-23PR.5-0.5-v0.2.0.csv}{n_geom}{Lambda_avg}
    {\( \Ngeom \)}{\( \lvert \Lambda_{\tavg} \rvert \)}
    {\( \vert \Lambda_{\tavg} \rvert \) vs \( \Ngeom \)}
  \caption{Scatter plot of \( \lvert \Lambda_{\tavg} \rvert \) versus \( \Ngeom \) for \( \alpha=0.5 \) on a log-log scale.}
  \label{fig:lambda-avg-prim}
\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdaavg-23PR.5-0.015625-v0.2.0.csv}{n_geom}{Lambda_avg}
    {\( \Ngeom \)}{\( \lvert \Lambda_{\tavg} \rvert \)}
    {\( \vert \Lambda_{\tavg} \rvert \) vs \( \Ngeom \)}
  \caption{Scatter plot of \( \lvert \Lambda_{\tavg} \rvert \) versus \( \Ngeom \) for \( \alpha=0.015625 \) on a log-log scale.}
  \label{fig:lambda-avg-prim015625}
\end{figure}
...
\midrule
\endhead
0 & 1.067e+00 & 4.989e-01 & 1.870e-01 & 2.638e-01 & 3.178e-01 & 3.602e-01 & 2.351e-01 & 0.0\% \\
...
Under HL-A, predictions and measurements converge to the same limit. For very large \( n \), minima should approach \( 2C_2 \), where \( C_2 \) is the twin prime constant. Figure~\ref{fig:lambda-minprim} shows \( \Lambda_{\min} \to 0 \) as \( n \)  grows.

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdamin-23PR.5-1-v0.2.0.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min} \rvert \)}{\( |\Lambda_{\min}| \) vs \( n_0 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\min} \rvert \) versus \( n_0 \) for \( \alpha=1 \) on a log-log scale.}
  \label{fig:lambda-minprim1}
\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdaalign-23PR.5-1-v0.2.0.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min}^{\talign} \rvert \)}{\( |\Lambda_{\min}^{\talign}| \) vs \( n_0 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\min}^{\talign} \rvert \) versus \( n_0 \) for \( \alpha=1 \) on a log-log scale.}
  \label{fig:lambda-alignprim1}
\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdabound-23PR.5-1-v0.2.0.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min}^{\text{bound}} \rvert \)}{\( |\Lambda_{\min}^{\text{bound}}| \) vs \( n_0 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\min}^{\text{bound}} \rvert \) versus \( n_0 \) for \( \alpha=1 \) on a log-log scale.}
  \label{fig:lambda-boundprim1}
\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdamin-23PR.5-0.5-v0.2.0.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min} \rvert \)}{\( |\Lambda_{\min}| \) vs \( n_0 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\min} \rvert \) versus \( n_0 \) for \( \alpha=0.5 \) on a log-log scale.}
  \label{fig:lambda-minprim}
\end{figure}

% \begin{figure}[h]
 % \centering
 % \FigLambdaAbsSignSplit{lambdamin-23PR.5-0.015625-v0.2.0.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min} \rvert \)}{\( |\Lambda_{\min}| \) vs \( n_0 \) }
  %\caption{Scatter plot of \( \lvert \Lambda_{\min} \rvert \) versus \( n_0 \) for \( \alpha=0.015625 \) on a log-log scale.}
  %\label{fig:lambda-minprim015625}
%\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdaalign-23PR.5-0.5-v0.2.0.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min}^{\talign} \rvert \)}{\( |\Lambda_{\min}^{\talign}| \) vs \( n_0 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\min}^{\talign} \rvert \) versus \( n_0 \) for \( \alpha=0.5 \) on a log-log scale.}
  \label{fig:lambda-alignprim}
\end{figure}


\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdabound-23PR.5-0.5-v0.2.0.csv}{n_0}{Lambda_min}{\( n_0 \)}{\( \lvert \Lambda_{\min}^{\text{bound}} \rvert \)}{\( |\Lambda_{\min}^{\text{bound}}| \) vs \( n_0 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\min}^{\text{bound}} \rvert \) versus \( n_0 \) for \( \alpha=0.5 \) on a log-log scale.}
  \label{fig:lambda-boundprim}
\end{figure}
...
\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdamax-23PR.5-1-v0.2.0.csv}{n_1}{Lambda_max}{\( n_1 \)}{\( \lvert \Lambda_{\max} \rvert \)}{\( |\Lambda_{\max}| \) vs \( n_1 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\max} \rvert \) versus \( n_1 \) for \( \alpha = 1 \) on a log-log scale.}
  \label{fig:lambda-maxprim1}
\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdamax-23PR.5-0.5-v0.2.0.csv}{n_1}{Lambda_max}{\( n_1 \)}{\( \lvert \Lambda_{\max} \rvert \)}{\( |\Lambda_{\max}| \) vs \( n_1 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\max} \rvert \) versus \( n_1 \) for \( \alpha = 0.5 \) on a log-log scale.}
  \label{fig:lambda-maxprim}
\end{figure}

\begin{figure}[h]
  \centering
  \FigLambdaAbsSignSplit{lambdamax-23PR.5-0.015625-v0.2.0.csv}{n_1}{Lambda_max}{\( n_1 \)}{\( \lvert \Lambda_{\max} \rvert \)}{\( |\Lambda_{\max}| \) vs \( n_1 \) }
  \caption{Scatter plot of \( \lvert \Lambda_{\max} \rvert \) versus \( n_1 \) for \( \alpha = 0.015625 \) on a log-log scale.}
  \label{fig:lambda-maxprim015625}
\end{figure}
...
\begin{equation}\label{eq:HLA-main-term-2}
...
\eqref{eq:analytic-lower-bound-global-2} establishes a valid global
...
\begin{equation}
  ...
  \ \text{ and }\
  \nprodstar=\xMertens .
  \label{eq:analytic-lower-bound-global-2}
\end{equation}
\end{proof}
...
\begin{equation}\label{eq:major-arc}
...
\medskip
Combining \eqref{eq:R2-int-again}, \eqref{eq:major-arc}, and \eqref{eq:major-arc-again},
\begin{equation}
...
\begin{equation}\label{eq:major-arc-again-copy}
\int_{\mathfrak m} S(\alpha)^2 e(-N\alpha)\,d\alpha\ \ll_{A,\varepsilon}\ \frac{N}{(\log N)^A}
\quad\text{for some }A>2,
\end{equation}
with \(S(\alpha)\) as in \eqref{eq:S-alpha}. The full hypothesis \eqref{eq:SIBV} is a convenient sufficient condition for \eqref{eq:major-arc-again-copy}, but it is not necessary. Any of the following implies \eqref{eq:major-arc-again-copy} and hence the corollary:
...
\end{equation}
Via Vaughan’s identity, Cauchy-Schwarz and the large sieve, this delivers \eqref{eq:major-arc-again-copy}.
...
\]
This yields \eqref{eq:major-arc-again-copy} after integrating over \(x\) and summing dyadically.
...
Assume the minor-arc input \eqref{eq:major-arc-again-copy} with some \(A>2\) (e.g. under SI--BV\(_\theta\)).
...
\begin{equation}\label{eq:S-alpha-copy}
...
\caption[]{Normalized by \( \frac{log^2 n}{M} \) Per-Decade HL-A Predictions for Goldbach Pair Counts for \( |m| \in [ 1, \lfloor \frac{n}{2} \rfloor ] \)}\label{tab:normalized-status-predictions} \\
\toprule
\multicolumn{1}{c}{\textbf{Dec.}} &
...
0 & 5 & 4.26609329 & 5 & 4.26609329 & 5 & 4.26609329 \\
0 & 6 & 6.21887455 & 6 & 6.21887455 & 6 & 6.21887455 \\
0 & 7 & 3.39303648 & 7 & 3.39303648 & 7 & 3.39303648 \\
0 & 8 & 2.81463781 & 8 & 2.81463781 & 8 & 2.81463781 \\
0 & 9 & 5.82036707 & 9 & 5.82036707 & 9 & 5.82036707 \\
