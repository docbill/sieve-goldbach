...
% In your document, before the figure (adjust path as needed)
...
Let \(N_\ast\) denote the smallest \(N_0\) such that \(C_{\min}(n) \ge \CminusProduct(n)\) for all subsequent \(n\) in our record.
...
remove \(100\%\) of candidates—hence \(C_{\min}\) may fall below the asymptotic floor until \(n\) is large enough
...
\noindent\emph{Notes.}
(i) Because we only record minima, this is conservative: any unrecorded intermediate values lie \emph{above} \(C_{\min}\).
