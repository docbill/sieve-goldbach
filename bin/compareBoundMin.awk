...
FNR==1 {
    ...
    if (format == "v0.2.0") {
        ...
        col_cbound = find_column($0, "CboundMin(n_a)")
        ...
        if (col_cbound == 0) { print "ERROR: CboundMin(n_a) column not found for " VERSION > "/dev/stderr"; missing_columns++ }
        ...
        col_minat_2 = find_column($0, "MIN AT")
        col_n0_2 = find_column($0, "n_0")
        col_cmin_2 = find_column($0, "Cpred_min")
        col_ngeom_2 = find_column($0, "N_geom")
        col_nbound = find_column($0, "n_a")
        col_cbound = find_column($0, "CboundMin")
        col_jitter = find_column($0, "jitter")
    ...
    if(col_label2 == 1) {
       ...
       print "START","n_0","C_min","Npred_0","CboundMin","Lambda_min","Jitter","JitterRatio","Status"
    }
    next
}

{
    ...
    # Determine status based on difference (reversed logic for minimum bounds)
    if (absd(c_diff) < 1e-10) {
        status = "EXACT"
    } else if (c_diff < 0) {
        status = "VIOLATION"  # Observed < predicted (violates lower bound)
    } else {
